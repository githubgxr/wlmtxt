		...
		if (xhr_day.readyState == 4 && xhr_day.status == 200) {
			...
			for (var i = 0; i < day_rank_response.length; i++) {
				...
				day_rank_str += '<a class="video_list_item_wrap" href="/wlmtxt/Works/Works_videoDetailsPage?accept_works.works_id='
						+ day_rank_response[i].works.works_id + '">';
				day_rank_str += '<div class="video_cover">';
				day_rank_str += '<img class="video_img" src="/wlmtxt/Works/Works_getImg?imgName='
						+ day_rank_response[i].works.works_cover + '"/>';
				...
				/* 标题 */
				day_rank_str += '<div class="video_title">'
						+ day_rank_response[i].works.works_title + '</div>';
				/* 浏览量 */
				day_rank_str += '<div class="video_hot">'
						+ day_rank_response[i].hot + '</div>';
				/* 用户 */
				day_rank_str += '<div class="video_username">'
						+ day_rank_response[i].user.user_username + '</div>';
				day_rank_str += '</div>';
				day_rank_str += '</a>';
				 ...
				 * day_rank_str+='<a class="video_label_item">绝地求生</a> <a
				 * class="video_label_item">绝地求生</a>';
				 */

				for (var num = 0; num < day_rank_response[i].keyWordDTOList.length; num++) {
					day_rank_str += '<a class="video_label_item">'
							+ day_rank_response[i].keyWordDTOList[num].keyword.keyword_name
							+ '</a>';
				}
				day_rank_str += '</div>';
				day_rank_str += '</div>';
	...
	xhr_week.onreadystatechange = function() {
		if (xhr_week.readyState == 4 && xhr_week.status == 200) {
			...
			for (var i = 0; i < long_week; i++) {
				...
				/*---视频链接---*/
				week_rank_str += '<a class="video_list_item_wrap" href="/wlmtxt/Works/Works_videoDetailsPage?accept_works.works_id='
						+ week_rank_response[i].works.works_id + '">';
				week_rank_str += '<div class="video_cover">';
				week_rank_str += '<img class="video_img" src="/wlmtxt/Works/Works_getImg?imgName='
						+ week_rank_response[i].works.works_cover + '"/>';
				...
				/* 标题 */
				week_rank_str += '<div class="video_title">'
						+ week_rank_response[i].works.works_title + '</div>';
				/* 浏览量 */
				week_rank_str += '<div class="video_hot">'
						+ week_rank_response[i].hot + '</div>';
				/* 用户 */
				week_rank_str += '<div class="video_username">'
						+ week_rank_response[i].user.user_username + '</div>';
				week_rank_str += '</div>';
				week_rank_str += '</a>';
				...
				for (var num = 0; num < week_rank_response[i].keyWordDTOList.length; num++) {
					week_rank_str += '<a class="video_label_item">'
							+ week_rank_response[i].keyWordDTOList[num].keyword.keyword_name
							+ '</a>';
				}
				week_rank_str += '</div>';
				week_rank_str += '</div>';
	...
	xhr_month.onreadystatechange = function() {
		if (xhr_month.readyState == 4 && xhr_month.status == 200) {
			...
			for (var i = 0; i < long_month; i++) {
				...
				/*---视频链接---*/
				month_rank_str += '<a class="video_list_item_wrap" href="/wlmtxt/Works/Works_videoDetailsPage?accept_works.works_id='
						+ month_rank_response[i].works.works_id + '">';
				month_rank_str += '<div class="video_cover">';
				month_rank_str += '<img class="video_img" src="/wlmtxt/Works/Works_getImg?imgName='
						+ month_rank_response[i].works.works_cover + '"/>';
				...
				/* 标题 */
				month_rank_str += '<div class="video_title">'
						+ month_rank_response[i].works.works_title + '</div>';
				/* 热度 */
				month_rank_str += '<div class="video_hot">'
						+ month_rank_response[i].hot + '</div>';
				/* 用户 */
				month_rank_str += '<div class="video_username">'
						+ month_rank_response[i].user.user_username + '</div>';
				month_rank_str += '</div>';
				month_rank_str += '</a>';
				...
				for (var num = 0; num < month_rank_response[i].keyWordDTOList.length; num++) {
					month_rank_str += '<a class="video_label_item">'
							+ month_rank_response[i].keyWordDTOList[num].keyword.keyword_name
							+ '</a>';
				}
				month_rank_str += '</div>';
				month_rank_str += '</div>';
