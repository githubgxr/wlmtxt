	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			if (list_appreciateNotice.length > 0) {
					...
					if (e == list_appreciateNotice[i].notification.notification_type) {
						...
						if (e == 1) {
							...
							appreciateNotice += '<div class="thumd_time">'
									+ list_appreciateNotice[i].notification.notification_gmt_create
									+ '</div>';
							...
							appreciateNotice += '<img id="'
									...
									+ list_appreciateNotice[i].worksDTO.user.user_avatar
									+ '"/>';
							appreciateNotice += '</div>';
						...
						appreciateNotice += '<div class="info_concise">'
								+ list_appreciateNotice[i].notification.notification_content
								+ '</div>';
				...
				$(".llls_content ")
						.html(
								'<img src="/wlmtxt/img/no_message_bg.png" style="width:250px;height:250px;margin:30px 240px 0px 240px;float:left;"/><div style="color:#99a2aa;font-size：16px;text-align:center;margin:0 0 50px 0;">该内容还没有通知消息哦！</div>');
			}
