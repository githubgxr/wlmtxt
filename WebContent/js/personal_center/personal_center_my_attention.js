		...
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			if (list_myAttention.length > 0) {
				...
				for (var i = 0; i < list_myAttention.length; i++) {
					...
					/* 用户头像 */
					myAttention += '<img class="user_img" id="'
							+ list_myAttention[i].user.user_id
							+ '" onclick="to_other_data(this.id)" src="/wlmtxt/Works/Works_getImg?imgName='
							+ list_myAttention[i].user.user_avatar + '"/>';
					myAttention += '</div>';
					...
					myAttention += '<div  id="'
							...
							+ list_myAttention[i].user.user_username + '</div>';
					/* 取消关注 */
					myAttention += '<div class="user_operateing" onclick="cancelFocus(this.id)" id="'
							+ list_myAttention[i].user.user_id + '">取消关注</div>';
					/* 个性签名 */
					myAttention += '<div class="user_grjj">'
							+ list_myAttention[i].user.user_bio + '</div>';
					myAttention += '</div>';
					myAttention += '</li>';
